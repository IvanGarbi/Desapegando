(function (factory) {
    ...
    const newCustomersChartsInit = (novosCondominos7Dias) => {
    ...
    const topCouponsChartInit = (totalProdutosVDisDes7Dias) => {
    ...
    const totalSalesChartInit = (totalVendasPeriodo, totalVendasPeriodo2) => {
        ...
        //    new Date().setDate(date.getDate() - 0),
    ...
    const payingCustomerChartInit = (totalNovasCampanhas30Dias) => {
        const { getData, getColor } = window.phoenix.utils;
                ...
                series: [
                    {
                        ...
                        data: [
                            {
                                value: totalNovasCampanhas30Dias[0]
                            }
                        ]
                    }
                ]
    ...
    const totalOrdersChartInit = (totalVendas) => {
        const { getColor, getData, getDates } = window.phoenix.utils;
                ...
                series: [
                    {
                        ...
                        data: [totalVendas[0], totalVendas[1], totalVendas[2], totalVendas[3], totalVendas[4], totalVendas[5], totalVendas[6]],
                        showBackground: true,
                        symbol: 'none',
                        itemStyle: {
                            borderRadius: 10
    ...
    docReady(totalSalesChartInit(totalVendasPeriodo, totalVendasPeriodo2));
    docReady(newCustomersChartsInit(novosCondominos7Dias));
    docReady(topCouponsChartInit(totalProdutosVDisDes7Dias));
...
function val() {
    ...
    if (d == 2) { // segundo valor escolhido
        totalSalesChartInit2(totalVendasPeriodo, totalVendasPeriodo3);
    }
    else if (d == 3) { // último valor escolhido
        totalSalesChartInit3(totalVendasPeriodo, totalVendasPeriodo4);
    }
    else { // primeiro valor escolhido
...
const totalSalesChartInit2 = (totalVendasPeriodo, totalVendasPeriodo2) => {
    const { getColor, getData, getDates } = window.phoenix.utils;
    const $totalSalesChart = document.querySelector('.echart-total-sales-chart');
    ...
    const tooltipFormatter = params => {
        const currentDate = window.dayjs(params[0].axisValue);
        const prevDate = window.dayjs(params[0].axisValue).subtract(2, 'month');
...
const totalSalesChartInit3 = (totalVendasPeriodo, totalVendasPeriodo2) => {
    const { getColor, getData, getDates } = window.phoenix.utils;
    const $totalSalesChart = document.querySelector('.echart-total-sales-chart');
    ...
    const tooltipFormatter = params => {
        const currentDate = window.dayjs(params[0].axisValue);
        const prevDate = window.dayjs(params[0].axisValue).subtract(3, 'month');
